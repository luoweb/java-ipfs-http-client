...
public class IPFS {
    ...
    public List<MerkleNode> add(List<NamedStreamable> files, boolean wrap, boolean hashOnly) throws IOException {
        Multipart m = new Multipart(protocol + "://" + host + ":" + port + version + "add?stream-channels=true&w="
                + wrap + "&n=" + hashOnly, "UTF-8");
        ...
        ;
        String res = m.finish();
        return JSONParser.parseStream(res).stream().map(x -> MerkleNode.fromJSON((Map<String, Object>) x))
                .collect(Collectors.toList());
    }

    public List<MerkleNode> ls(Multihash hash) throws IOException {
        Map reply = retrieveMap("ls?arg=" + hash);
        return ((List<Object>) reply.get("Objects")).stream()
    ...
    public List<Multihash> refs(Multihash hash, boolean recursive) throws IOException {
        ...
        return JSONParser.parseStream(jsonStream).stream().map(m -> (String) (((Map) m).get("Ref"))).map(Cid::decode)
                .collect(Collectors.toList());
    }
    ...
    public class Refs {
        public List<Multihash> local() throws IOException {
            ...
            return JSONParser.parseStream(jsonStream).stream().map(m -> (String) (((Map) m).get("Ref")))
    ...
    public class Pin {
        ...
        public Map<Multihash, Object> ls(PinType type) throws IOException {
            return ((Map<String, Object>) (((Map) retrieveAndParse("pin/ls?stream-channels=true&t=" + type.name()))
                    .get("Keys"))).entrySet().stream()
        ...
        public List<Multihash> rm(Multihash hash, boolean recursive) throws IOException {
            ...
            return ((List<Object>) json.get("Pins")).stream().map(x -> Cid.decode((String) x))
                    .collect(Collectors.toList());
        }
        ...
        public List<MultiAddress> update(Multihash existing, Multihash modified, boolean unpin) throws IOException {
            return ((List<Object>) ((Map) retrieveAndParse(
                    "pin/update?stream-channels=true&arg=" + existing + "&arg=" + modified + "&unpin=" + unpin))
                            .get("Pins")).stream().map(x -> new MultiAddress((String) x)).collect(Collectors.toList());
        }
    }
    ...
    public class Key {
        public KeyInfo gen(String name, Optional<String> type, Optional<String> size) throws IOException {
            return KeyInfo.fromJson(retrieveAndParse("key/gen?arg=" + name + type.map(t -> "&type=" + t).orElse("")
                    + size.map(s -> "&size=" + s).orElse("")));
        }

        public List<KeyInfo> list() throws IOException {
            return ((List<Object>) ((Map) retrieveAndParse("key/list")).get("Keys")).stream().map(KeyInfo::fromJson)
                    .collect(Collectors.toList());
        }
        ...
        public List<KeyInfo> rm(String name) throws IOException {
            return ((List<Object>) ((Map) retrieveAndParse("key/rm?arg=" + name)).get("Keys")).stream()
                    .map(KeyInfo::fromJson).collect(Collectors.toList());
        }
    }
    ...
    public class Pubsub {
         ...
         * @throws IOException
         */
        public void sub(String topic, Consumer<Map<String, Object>> results, Consumer<IOException> error)
                throws IOException {
    ...
    public class Block {
        ...
        public List<MerkleNode> put(List<byte[]> data, Optional<String> format) throws IOException {
            // N.B. Once IPFS implements a bulk put this can become a single multipart call
            // with multiple 'files'
        ...
        public MerkleNode put(byte[] data, Optional<String> format) throws IOException {
            ...
            Multipart m = new Multipart(
                    protocol + "://" + host + ":" + port + version + "block/put?stream-channels=true" + fmt, "UTF-8");
            try {
     ...
     * {Object} is a subset of {Block}
     */
    public class IPFSObject {
        public List<MerkleNode> put(List<byte[]> data) throws IOException {
            Multipart m = new Multipart(
                    protocol + "://" + host + ":" + port + version + "object/put?stream-channels=true", "UTF-8");
            for (byte[] f : data)
        ...
        public List<MerkleNode> put(String encoding, List<byte[]> data) throws IOException {
            ...
            Multipart m = new Multipart(protocol + "://" + host + ":" + port + version
                    + "object/put?stream-channels=true&encoding=" + encoding, "UTF-8");
            for (byte[] f : data)
                m.addFilePart("file", Paths.get(""), new NamedStreamable.ByteArrayWrapper(f));
            String res = m.finish();
            return JSONParser.parseStream(res).stream().map(x -> MerkleNode.fromJSON((Map<String, Object>) x))
        ...
        public MerkleNode _new(Optional<String> template) throws IOException {
            ...
            Map json = retrieveMap(
                    "object/new?stream-channels=true" + (template.isPresent() ? "&arg=" + template.get() : ""));
            return MerkleNode.fromJSON(json);
        }

        public MerkleNode patch(Multihash base, String command, Optional<byte[]> data, Optional<String> name,
                Optional<Multihash> target) throws IOException {
            ...
            switch (command) {
                ...
                case "append-data":
                    ...
                    Multipart m = new Multipart(protocol + "://" + host + ":" + port + version + "object/patch/"
                            + command + "?arg=" + base.toBase58() + "&stream-channels=true", "UTF-8");
    ...
    public class DHT {
        public List<Map<String, Object>> findprovs(Multihash hash) throws IOException {
            return getAndParseStream("dht/findprovs?arg=" + hash).stream().map(x -> (Map<String, Object>) x)
                    .collect(Collectors.toList());
        }
    ...
    public List<MultiAddress> bootstrap() throws IOException {
        return ((List<String>) retrieveMap("bootstrap/").get("Peers")).stream().flatMap(x -> {
            try {
    ...
    public class Bootstrap {
        ...
        public List<MultiAddress> add(MultiAddress addr) throws IOException {
            return ((List<String>) retrieveMap("bootstrap/add?arg=" + addr).get("Peers")).stream()
                    .map(x -> new MultiAddress(x)).collect(Collectors.toList());
        }
        ...
        public List<MultiAddress> rm(MultiAddress addr, boolean all) throws IOException {
            return ((List<String>) retrieveMap("bootstrap/rm?" + (all ? "all=true&" : "") + "arg=" + addr).get("Peers"))
     ...
     * component that opens, listens for, and maintains connections to other ipfs
     * peers in the internet.
     */
    public class Swarm {
        public List<Peer> peers() throws IOException {
            Map m = retrieveMap("swarm/peers?stream-channels=true");
            return ((List<Object>) m.get("Peers")).stream().flatMap(json -> {
                try {
        ...
        public Map<Multihash, List<MultiAddress>> addrs() throws IOException {
            ...
            return ((Map<String, Object>) m.get("Addrs")).entrySet().stream().collect(Collectors.toMap(
                    e -> Multihash.fromBase58(e.getKey()),
                    e -> ((List<String>) e.getValue()).stream().map(MultiAddress::new).collect(Collectors.toList())));
        }
    ...
    public class Dag {
        ...
        public MerkleNode put(String inputFormat, byte[] object, String outputFormat) throws IOException {
            ...
            Multipart m = new Multipart(
                    prefix + "dag/put/?stream-channels=true&input-enc=" + inputFormat + "&f=" + outputFormat, "UTF-8");
    ...
    public class Config {
        ...
        public void replace(NamedStreamable file) throws IOException {
            Multipart m = new Multipart(
                    protocol + "://" + host + ":" + port + version + "config/replace?stream-channels=true", "UTF-8");
            m.addFilePart("file", Paths.get(""), file);
            String res = m.finish();
        }
    ...
    private Stream<Object> retrieveAndParseStream(String path, ForkJoinPool executor) throws IOException {
        ...
        executor.submit(() -> getObjectStream(in, res -> {
     ...
     * @throws IOException
     */
    private void retrieveAndParseStream(String path, Consumer<Object> results, Consumer<IOException> err)
            throws IOException {
    ...
    private static byte[] get(URL target, int timeout) throws IOException {
        ...
        try {
            ...
            String err = Optional.ofNullable(conn.getErrorStream()).map(s -> new String(readFully(s)))
                    .orElse(e.getMessage());
            throw new RuntimeException("IOException contacting IPFS daemon.\nTrailer: "
                    + conn.getHeaderFields().get("Trailer") + " " + err, e);
        }
    }
